...
x_opts = zeros(length(data_list),13);
...
V_array=zeros(1,length(SoC)); % OCV array for each SoC
...
OCV=[Vt(1)+(I)*(sum(x_opts(10,1:5))+x_opts(10,12)), NaN, Vt(3)+(I)*(sum(x_opts(9,1:5))+x_opts(9,12)), ... 
    Vt(4)+(I)*(sum(x_opts(8,1:5))+x_opts(8,12)), Vt(5)+(I)*(sum(x_opts(7,1:5))+x_opts(7,12)), ...
    Vt(6)+(I)*(sum(x_opts(6,1:5))+x_opts(6,12)), Vt(7)+(I)*(sum(x_opts(5,1:5))+x_opts(5,12)), ...
    Vt(8)+(I)*(sum(x_opts(4,1:5))+x_opts(4,12)), Vt(9)+(I)*(sum(x_opts(3,1:5))+x_opts(3,12)), ...
    Vt(10)+(I)*(sum(x_opts(2,1:5))+x_opts(2,12)), Vt(11)+(I)*(sum(x_opts(1,1:5))+x_opts(1,12))];
...
y=spline(SoC, V_array, x); % Spline interpolation of SoC curve
...
function [x_opt, RMSEI, RMSEII]=fit(Zdata, frequencydata, SoC, display)
    ...
    opts = optimoptions('lsqnonlin', 'Display', 'iter', 'FunctionTolerance', 1e-12, 'OptimalityTolerance', 1e-12);
    ...
    [x_opt,resnorm] = run(ms,problem,200);
